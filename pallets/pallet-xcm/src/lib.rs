        ...
        #[pallet::weight({
			match ((*assets.clone()).try_into(), (*dest.clone()).try_into()) {
				(Ok(assets), Ok(dest)) => {
					...
					T::Weigher::weight(&mut message).map_or(Weight::MAX, |w| T::WeightInfo::reserve_withdraw_assets().saturating_add(w))
				},
				_ => Weight::MAX,
			}
		})]
...
impl<T: Config> Pallet<T> {
    ...
    pub fn request_version_notify(dest: impl Into<MultiLocation>) -> XcmResult {
        ...
        let instruction = SubscribeVersion {
            query_id,
    ...
    fn enact(self) -> Result<(), xcm_executor::traits::LockError> {
        ...
        match self.item_index {
            Some(index) => {
                    ...
                    locks[index].1.try_as::<_>() == Ok(&self.unlocker),
                    UnexpectedState
                );
...
impl<T: Config> xcm_executor::traits::AssetLock for Pallet<T> {
    ...
    fn prepare_lock(
        ...
        let item_index = locks
            .iter()
            .position(|x| x.1.try_as::<_>() == Ok(&unlocker));
        ensure!(
            item_index.is_some() || locks.len() < T::MaxLockers::get() as usize,
            NoResources
        );
        Ok(LockTicket {
            sovereign_account,
    ...
    fn note_unlockable(
        ...
        let mut record = RemoteLockedFungibleRecord {
            amount,
            owner,
            locker,
            users: 0,
        ...
        if let Some(old) = RemoteLockedFungibles::<T>::get(&key) {
            // Make sure that the new record wouldn't clobber any old data.
                ...
                old.locker == record.locker && old.owner == record.owner,
                WouldClobber
            );
    ...
    fn prepare_reduce_unlockable(
        ...
        // Make sure that the record contains what we expect and there's enough to unlock.
            ...
            locker == record.locker && owner == record.owner,
            WouldClobber
        );
...
impl<T: Config> OnResponse for Pallet<T> {
    ...
    fn on_response(
        ...
        match (response, Queries::<T>::get(query_id)) {
            (
                Response::Version(v),
                Some(QueryStatus::VersionNotifier {
